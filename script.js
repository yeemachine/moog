...
let svgPath1 =
  "M2215.46,1305c-677.2,51-1353.67,51-2034.12,0-49.38-3.7-104-52.5-113.59-97.9-75.38-357.2-86.33-709.43,0-1064.14C80,93.27,132,48.84,181.34,45,861.79-7.46,1536.76-6,2215.46,45c49.38,3.71,101.76,48.13,113.59,97.9,85.16,358.08,83.66,713.56,0,1064.14C2317.18,1256.83,2264.8,1300.76,2215.46,1305Z";
let svgPath2 =
  "M2304,1347c-46,2-2023,0-2211,0-48.25,0-88.5-42.75-89-90C.14,892,1.72,458.06,4,93,4.33,39.67,44,3.72,93,3c68-1,2117-1,2211,0,53,.56,88.28,40,90,90,3,87,3,1018,0,1164C2393,1308,2352.45,1344.89,2304,1347Z";
...
app.ticker.add(delta => {
  ...
  if (theremin) {
    if (gameActive && theremin.scale.x < stageScale * 1.15) {
      theremin.scale = new PIXI.Point(
        theremin.scale.x + stageScale * 0.0125,
        theremin.scale.y + stageScale * 0.0125
      );
      theremin.position.set(
        (WIDTH - theremin.width) / 1.8,
        (HEIGHT - theremin.height) / 2
      );

      fg.scale = new PIXI.Point(
        theremin.scale.x + stageScale * 0.015,
        theremin.scale.y + stageScale * 0.015
      );
      fg.position.set((WIDTH - fg.width) / 2, (HEIGHT - fg.height) / 1.8);

      if (bgm) {
        bg2.scale = new PIXI.Point(
          bg2.scale.x - stageScale * 0.0025,
          bg2.scale.y - stageScale * 0.0025
        );
        bg2.position.x = (WIDTH - bg2.width) / 2;
      } else {
        bg.scale = new PIXI.Point(
          bg.scale.x - stageScale * 0.0025,
          bg.scale.y - stageScale * 0.0025
        );
        bg.position.x = (WIDTH - bg.width) / 2;
      }
    } else if (!gameActive && theremin.scale.x > stageScale * 0.99) {
      ...
      fg.scale = new PIXI.Point(
        theremin.scale.x - stageScale * 0.02,
        theremin.scale.y - stageScale * 0.02
      );
  ...
  if (warmLightContainer.children[0]) {
    ...
    warmLightContainer.children[1].brightness = 1 + Math.cos(time - 0.8) * 1;
    warmLightContainer.children[2].brightness = 1 + Math.cos(time - 1.6) * 1;
    warmLightContainer.children[3].brightness = 1 + Math.cos(time - 0.4) * 1;
  }
...
graphics.parentGroup = PIXI.lights.diffuseGroup;

PIXI.loader
  .add(
    "bg_diffuse",
    "https://cdn.glitch.com/e352d3ca-2e03-47f1-acfd-675dff041f5f%2FNogoogleBG.jpg?v=1580140271561"
  )
  .add(
    "bg_normal",
...
function onAssetsLoaded(loader, res) {
  ...
  play.position.set(WIDTH * 0.4, HEIGHT * 0.3);
  playLight.position.x = play.width / 2;
  playLight.position.y = play.height / 2;
  pause.position.set(WIDTH * 0.015, HEIGHT * 0.74);
  songButton.position.set(WIDTH * 0.02, HEIGHT * 0.025);
  videoButton.position.set(WIDTH * 0.925, HEIGHT * 0.75);
  ...
  [knob1, knob2, knob3, knob4].forEach(e => {
    ...
    e.x = e.width / 2;
    e.y = e.height / 2;
    e.rotation = -0.8 * Math.PI;
    e.parentGroup = PIXI.lights.diffuseGroup;
    theremin.addChild(e);
  });

  knob1.position.set(
    theremin.children[0].width * 0.376,
    theremin.children[0].height * 0.8
  );
  knob2.position.set(
    theremin.children[0].width * 0.642,
    theremin.children[0].height * 0.8
  );
  knob3.position.set(
    theremin.children[0].width * 0.695,
    theremin.children[0].height * 0.8
  );
  knob4.position.set(
    theremin.children[0].width * 0.695,
  ...
  let warmLights = [
    {
      x: 0.13039276123046876 * WIDTH,
      y: 0.4809034459731158 * HEIGHT,
      color: 0xff0000
    },
    {
      x: 0.40115692138671877 * WIDTH,
      y: 0.3336615169749541 * HEIGHT,
      color: 0xff0000
    },
    {
      x: 0.9425523885091146 * WIDTH,
      y: 0.5915714039522059 * HEIGHT,
      color: 0xff0000
    },
    {
      x: 0.7425523885091146 * WIDTH,
      y: 0.1915714039522059 * HEIGHT,
      color: 0xff0000
    },

    {
      x: 0.8194022623697916 * WIDTH,
      y: 0.7614661721622242 * HEIGHT,
      color: 0xff7f00
    },
    {
      x: 0.8256411743164063 * WIDTH,
      y: 0.0489926955279182 * HEIGHT,
      color: 0xff7f00
    },
    {
      x: 0.11977320353190105 * WIDTH,
      y: 0.8508505428538603 * HEIGHT,
      color: 0xff7f00
    },
    {
      x: 0.33254694620768227 * WIDTH,
      y: 0.9263327205882353 * HEIGHT,
      color: 0xff7f00
    },
    {
      x: 0.7407407633463542 * WIDTH,
      y: 0.9326670927159927 * HEIGHT,
      color: 0xff7f00
    },
    {
      x: 0.6591830444335938 * WIDTH,
      y: 0.7735333610983456 * HEIGHT,
      color: 0xff7f00
    },
    {
      x: 0.39131688435872397 * WIDTH,
      y: 0.789508954216 * HEIGHT,
      color: 0xff7f00
    }
  ];
  ...
  document.fonts.load('10pt "America Mono"').then(() => {
    ...
    displayText.position.set(
      theremin.children[0].texture.width * 0.513,
      theremin.children[0].texture.height * 0.762
    );
    displayText.alpha = 0.3;
    ...
    displayText2.position.set(
      theremin.children[0].texture.width * 0.44,
      theremin.children[0].texture.height * 0.762
    );
    ...
    document.fonts.load('10pt "Pose Font"').then(() => {
      ...
      displayText3.position.set(
        theremin.children[0].texture.width * 0.562 - displayText3.width / 2,
        theremin.children[0].texture.height * 0.792 - displayText3.height / 2
      );
    ...
    songCred.position.set(WIDTH * 0.09, HEIGHT * 0.075 - songCred.height / 2);
    songCred.alpha = 0;
...
document
  .querySelector("nav .material-icons")
